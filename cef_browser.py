...
class CefBrowser():
    ...
    def __init__(self, parentFrame, closeCallback, browserConnectionErrorCallback):
...
class BrowserFrame(tk.Frame):
    ...
    def on_configure(self, event):
        if not self.browser:
            ...
            #cancel current pending page reload function call, if it exists
            if self.reloadPageId is not None:
                self.after_cancel(self.reloadPageId)

            # reload current browser page after 0.5 seconds
            self.reloadPageId = self.after(500, lambda:self.browser.Reload())
...
class LoadHandler(object):
    ...
    def OnLoadError(self, browser, frame, error_code, error_text_out, failed_url, **_):
        ...
        #print("error_code: " + str(error_code)+ " " + "text_out:" + str(error_text_out) + " failed_url:" + str(failed_url))
